...
Important note:
    The AVR ISP protocol dates back to very early devices.  Many of the parametric information included in the protocol
    metadata applies only to those old devices.  This is not a universal implementation, and does not draw any such
    information from the XML descriptions included in Atmel Studio (ATDF). It does work on ATmega328P and friends.
...
class AvrIspProtocol(Jtagice3Protocol):
    ...
    # Could be parameterised, but static on newer AVRs
